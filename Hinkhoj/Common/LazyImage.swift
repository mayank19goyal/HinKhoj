    ...
    private func saveImage(image:UIImage, imagePath:String) {
        ...
        do {
            ...
            if let actualError = error {
                HinkhojLogs("Image not saved. \(actualError)")
            }
        } catch {
            fatalError()
        }
                    ...
                    let task = session.dataTask(with: urlRequest, completionHandler: { (data, response, error) in
                        ...
                        if response != nil {
                            ...
                            if httpResponse.statusCode != 200 {
                                HinkhojLogs("LazyImage status code : \(httpResponse.statusCode)")
                        ...
                        if data == nil {
                            if error != nil {
                                HinkhojLogs("Error : \(error!.localizedDescription)")
                            }
                            HinkhojLogs("LazyImage: No image data available")
